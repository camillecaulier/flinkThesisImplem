...
public class TestMeanOperatorJavaMultiSource {
    ...
    public CompleteOperator<EventBasic> giveOperator(Class clazz, StreamExecutionEnvironment env, String javaSourceParameters){
        if (clazz == MeanBasic.class){
            ...
            return new MeanAggregateAware(javaSourceParameters, env, parallelism, 3, isJavaSource, sourceParallelism, aggregatorParallelism);
            ...
            return new MeanDChoices(javaSourceParameters, env, parallelism, isJavaSource, sourceParallelism, aggregatorParallelism);
        } else if (clazz == MeanWChoices.class){
            return new MeanWChoices(javaSourceParameters, env, parallelism, isJavaSource, sourceParallelism, aggregatorParallelism);
        } else if (clazz == MeanPKG.class){
            return new MeanPKG(javaSourceParameters, env, parallelism, isJavaSource, sourceParallelism, aggregatorParallelism);
        } else if (clazz == MeanTopKHashRoundRobin.class){
            return new MeanTopKHashRoundRobin(javaSourceParameters, env, parallelism, isJavaSource, sourceParallelism, aggregatorParallelism);
        } else if (clazz == MeanTopKCAMRoundRobin.class){
            return new MeanTopKCAMRoundRobin(javaSourceParameters, env, parallelism, 3, isJavaSource, sourceParallelism, aggregatorParallelism);
        }
        else{
            return null;
        }
